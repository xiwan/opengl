...
int lighting_basic()
{
	...
	Shader lampShader("./shaders/light_basic.shader.vs", "./shaders/light_lamp.shader.fs");
...
// -----------
	while (!glfwWindowShouldClose(window))
	{
		...
		view = basicCamera.GetViewMatrix();
		projection = glm::perspective(glm::radians(basicCamera.Zoom), (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
